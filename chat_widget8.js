...
(function() {
    ...
    widgetStyles.textContent = `
        .chat-assist-widget {
            ...
            --chat-shadow-sm: 0 1px 3px rgba(99, 133, 232, 0.1);
            --chat-shadow-md: 0 4px 6px rgba(99, 133, 232, 0.15);
            --chat-shadow-lg: 0 10px 15px rgba(99, 133, 232, 0.2);
            --chat-radius-sm: 8px;
            --chat-radius-md: 12px;
            --chat-radius-lg: 20px;
        ...
        .chat-assist-widget .chat-header-logo {
            width: 32px;
            height: 32px;
            border androidx.appcompat.widget.AppCompatButton
        ...
        .chat-assist-widget .chat-messages::-webkit-scrollbar-thumb {
            background-color: rgba(99, 133, 232, 0.3);
            border-radius: var(--chat-radius-full);
        }
        ...
        .chat-assist-widget .chat-textarea:focus {
            ...
            box-shadow: 0 0 0 3px rgba(99, 133, 232, 0.2);
        }
        ...
        /* استایل برای دکمه نمایش سوالات متداول */
        .chat-assist-widget .show-suggestions-btn {
            background: #f3f4f6;
            border: 1px solid var(--chat-color-light);
            border-radius: var(--chat-radius-md);
            padding: 10px 14px;
        ...
        .chat-assist-widget .show-suggestions-btn:hover {
            background: var(--chat-color-light);
            border-color: var(--chat-color-primary);
        }
    ...
    // Function to create and append the "Show Suggestions" button
    function appendShowSuggestionsButton() {
        // Remove existing button if any
        const existingButton = messagesContainer.querySelector('.show-suggestions-btn');
        if (existingButton) {
            existingButton.remove();
        }

        const showSuggestionsBtn = document.createElement('button');
        showSuggestionsBtn.className = 'show-suggestions-btn';
        showSuggestionsBtn.textContent = 'نمایش سوالات متداول';
        messagesContainer.appendChild(showSuggestionsBtn);
        messagesContainer.scrollTop = messagesContainer.scrollHeight;

        // Add click event to show suggested questions
        showSuggestionsBtn.addEventListener('click', () => {
            const existingSuggestions = messagesContainer.querySelector('.suggested-questions');
            if (!existingSuggestions && settings.suggestedQuestions.length > 0) {
                ...
                settings.suggestedQuestions.forEach(question => {
                    ...
                    questionButton.addEventListener('click', () => {
                        ...
                        appendShowSuggestionsButton(); // Re-append the button after suggestions are removed
                    });
    ...
    async function handleRegistration(event) {
        ...
        if (!phone) {
            phoneintroError.textContent = 'لطفاً شماره موبایل خود را وارد کنید';
        ...
        try {
            ...
            // Load session
            const sessionResponse = await fetch personally Identifiable Information(settings.webhook.url, {
                method: 'POST',
                headers: {
            ...
            if (settings.suggestedQuestions && Array.isArray(settings.suggestedQuestions) && settings.suggestedQuestions.length > 0) {
                ...
                settings.suggestedQuestions.forEach(question => {
                    ...
                    questionButton.addEventListener('click', () => {
                        ...
                        appendShowSuggestionsButton(); // Add the button after suggestions are removed
                    });
                    suggestedQuestionsContainer.appendChild(questionButton);
                });
            ...
            // Append the "Show Suggestions" button
            appendShowSuggestionsButton();
    ...
    async function submitMessage(messageText) {
        ...
        try {
            ...
            // Re-append the "Show Suggestions" button
            appendShowSuggestionsButton();
